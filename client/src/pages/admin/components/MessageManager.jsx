...
function MessageManager() {
    ...
    // ⭐ Lấy thêm onlineUsers từ context
    const { dataUser, newMessage, onlineUsers } = useStore();
    ...
    useEffect(() => {
        ...
        // Chỉ append vào khung chat nếu đang mở đúng conversation
        setMessages((prev) => {
            if (!selectedUser) return prev;
            // newMessage.conversation là id conversation (từ backend)
            if (newMessage.conversation?.toString() !== selectedUser._id?.toString()) return prev;
            return [...prev, newMessage];
        });

        // Gọi lại API để list bên trái được sort theo updatedAt (backend đã .sort({ updatedAt: -1 }))
        fetchConversations();
    ...
    const handleSelectUser = (user) => {
        setSelectedUser(user);
        // Reset badge chưa đọc cho cuộc trò chuyện này trên UI
        setUsers((prev) => prev.map((u) => (u._id === user._id ? { ...u, lengthIsRead: 0 } : u)));
    };
    ...
    const isAdminMessage = (message) => {
        return (
            ...
            message.sender?._id === dataUser._id ||
            message.sender === dataUser._id
        );
    };

    // ⭐ Helper lấy trạng thái online (ưu tiên realtime từ socket, fallback DB isOnline)
    const getOnlineStatus = (userId, fallbackIsOnline) => {
        if (!userId) return !!fallbackIsOnline;
        if (onlineUsers && Object.prototype.hasOwnProperty.call(onlineUsers, userId)) {
            return onlineUsers[userId];
        }
        return !!fallbackIsOnline;
    };

    // ⭐ Áp dụng search cho danh sách users
    const filteredUsers = users.filter((conversation) => {
        const name = conversation.user?.fullName || '';
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <div className="relative">
                                                    ...
                                                    {(() => {
                                                        const isOnline = getOnlineStatus(
                                                            selectedUser.user?._id?.toString(),
                                                            selectedUser.user?.isOnline,
                                                        );
                                                        return (
                                                            <span
